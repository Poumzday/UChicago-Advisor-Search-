...
mkdir -p "$RT/pages" "$LA"
cp "$SRC"/menubar_app.py "$SRC"/bus_app.py \
...
# Two separate menu-bar items (NBER digest + CTA #6 bus) + the weekly scrape.
...
# Remove retired agents (weather tab dropped; merged dashboard no longer used).
for L in com.poum.nberdigest.weather com.poum.nberdigest.dashboard; do
  launchctl bootout "$GUI/$L" 2>/dev/null || true
  rm -f "$LA/$L.plist"
done

for L in com.poum.nberdigest.menubar com.poum.nberdigest.bus com.poum.nberdigest; do
  launchctl bootout "$GUI/$L" 2>/dev/null || true
  launchctl bootstrap "$GUI" "$LA/$L.plist"
done

echo "==> Done."
echo "    Two menu-bar items: NBER digest and the CTA #6 bus."
